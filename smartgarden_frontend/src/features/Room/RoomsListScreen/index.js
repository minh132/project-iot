...
function RoomsListScreen(props) {
    ...
    const [selected, setSelected] = useState(() =>
        currentgarden.length > 0 ? currentgarden?.roomsList[0]?._id : ""
    );
    const deviceListOfCurrentRoom = devicesListOfgarden?.filter(
        (device) => device.roomId === selected
    );

    PusherHelper.Subscribe("device", "device-data", (d) => {
        const deviceData = d.deviceData;
        dispatch(updateDevicesListOfGarden(deviceData));
    });
                ...
                <div className="d-flex flex-column">
                    ...
                    >
                        <div className="d-flex flex-wrap">
                            {currentgarden?.roomsList?.map((room, index) => (
                                <div
                                    key={index}
