...
function ModalCreateDevice(props) {
    ...
    const roomOptions = [];
    for (let i = 0; i < currentgarden?.roomsList?.length; i++) {
        roomOptions.push({
            value: currentgarden?.roomsList[i]._id,
            text: currentgarden?.roomsList[i].roomName,
        });
    }
        ...
        onSubmit: async (values) => {
            ...
            try {
                ...
                if (res.payload.result === "failed") {
                    ...
                    await dispatch(
                        thunkGetRoomsList({ gardenId: currentgarden._id })
                    );
                }
            } catch (error) {
