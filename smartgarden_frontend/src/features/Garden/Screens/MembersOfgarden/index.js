...
MembersOfgarden.propTypes = {};

function MembersOfgarden(props) {
    const dispatch = useDispatch();

    const { currentAccount } = useSelector((state) => state?.auth);
    const { currentgarden, isDeletingMember } = useSelector(
        (state) => state?.garden
    );
    ...
    const memberList = currentgarden?.accountList?.filter(
        (account) => account.status === "owner"
    );
    const requestingList = currentgarden?.accountList?.filter(
...
export default MembersOfgarden;
