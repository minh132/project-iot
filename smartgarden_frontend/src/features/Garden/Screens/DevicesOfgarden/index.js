...
DevicesOfgarden.propTypes = {};

function DevicesOfgarden(props) {
    const dispatch = useDispatch();

    const { currentgarden } = useSelector((state) => state?.garden);
    const { isDeletingDevice, devicesListOfgarden } = useSelector(
        (state) => state?.device
    );
    ...
    const handleDeleteDevice = async () => {
        ...
        if (res.payload.result === "success") {
            ...
            await dispatch(thunkGetDevicesListOfGarden({ gardenId: currentgarden._id }));
        }
    };
                    ...
                    <div>
                        ...
                        {devicesListOfgarden?.length > 0 ? (
                            <div className="font-weight-bolder text-black-50 font-size-lg text-remaining">
                                Tổng cộng: {devicesListOfgarden?.length} thiết bị
                            </div>
                        ) : (
                            <div className="font-weight-bolder text-black-50 font-size-lg text-remaining">
                                Danh sách trống
                ...
                <div className="card-body p-0">
                    {devicesListOfgarden?.length > 0 && (
                        <BootstrapTable className="BootstrapTable" striped bordered hover>
                            ...
                            <tbody>
                                {devicesListOfgarden?.map((item, index) => (
                                    <tr key={index}>
                                        <td>{index + 1}</td>
...
export default DevicesOfgarden;
